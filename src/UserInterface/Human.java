...
public class Human extends Player{
	...
	private int getRaiseAmount(){
		try{
			playerAction = dataIn.readLine();
		}
		catch( IOException e ){
			System.out.println("Error!");
		}
		int raiseAmount = Integer.parseInt(playerAction);
		if(raiseAmount < minRaise){ //If the player raises less the the minimum allowed
			if(chips >= minRaise + costToCall){ //if the player can afford to raise the minimum
				raiseAmount = minRaise; //sets the raise amount to the miin allowed bet
			}
			else
				//if the player can't afford the min raise then bet however many chips theey have left
				raiseAmount = chips - costToCall;
		}
		return raiseAmount;
	}
